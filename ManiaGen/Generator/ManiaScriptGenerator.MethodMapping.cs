...
public partial class ManiaScriptGenerator
{
    private void GenerateNetMethodMapping()
    {
        ...
        {
            CreateNetMethodMapping(typeof(IEnumerable), ".[]", args =>
            {
                return Return(() => ElementAt(() => args[0], () => args[1]));
            }, new Type[]
            {
                typeof(IScriptValue)
            }, embed: true);
            
            CreateNetMethodMapping(typeof(object), "Yield", _ =>
            {
                Root.Statements.Add(new RawStatement("yield"));
                return Void.Default;
            }, new Type[]
            {
                typeof(Void)
        ...
        void GenerateString()
        {
            CreateNetMethodMapping(typeof(string), "Length", args =>
            {
                var lib = RequireLib<MsTextLib>();
                return Return(() => Method($"{lib.Name}::Length", new Func<IScriptValue>[]
                {
                    () => args[0]
                }, new Integer(0)
                {
